...
export default function Stopwatch({ displayVideo, StartStop, isRunning, setIsRunning }) {
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      interval = setInterval(() => {
        setMilliseconds(prevMillisecond => {
          const currentMillisecond = prevMillisecond + 10
          if (currentMillisecond >= 1000) {
            setSeconds(prevSeconds => prevSeconds + 1)
            return currentMillisecond - 1000
          }
  ...
  return (
    <div className='flex justify-center items-center mt-[35%] sm:mt-[10%]'>
